...
class TeamProblemStatus
    ...
    getContributingPenalty : ->
        return @cache.penalty ? @cache.penalty = do =>
            if this.isAccepted()
                return (this.getFailedAttempts()) * (@contest.penalty ? 20) + this.getSolvedTime()
            else return 0
        ...
        return @cache.penaltyMemo ? @cache.penaltyMemo = do =>
            if this.isAccepted()
                if this.getFailedAttempts() == 0
                    ...
                    return "#{this.getSolvedTime()} + #{@contest.penalty ? 20} * #{this.getFailedAttempts()} = #{this.getContributingPenalty()}"
            else return ''
