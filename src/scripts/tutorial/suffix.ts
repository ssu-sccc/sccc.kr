...
if (root) {
  ...
  function query() {
    ...
    get('[data-query-compare]').innerHTML = `<div>${m(String.raw`S_{${i}}=`)}${chars(i, q.value)}</div><div>${m(String.raw`S_{${j}}=`)}${chars(j, q.value)}</div>`;
    get('[data-range]').innerHTML = i === j ? `동일한 접미사: ${m(String.raw`N-i=${data.n}-${i}=${q.value}`)}. RMQ를 하지 않는다.` : `${m(String.raw`\operatorname{rank}[${i}]=${data.rank[i]},\quad\operatorname{rank}[${j}]=${data.rank[j]}`)}<br />${m(String.raw`\min_{${q.left+1}\le r\le ${q.right}}\mathrm{LCP}[r]=\min(${q.indices.map((r:number)=>data.lcp[r]).join(',')})=${q.value}`)}`;
    get('[data-reason]').innerHTML=`<h4>원문 좌표 → 정렬 좌표 → 사이의 간선</h4><div class="t-reason-grid"><div><small>첫 시작 위치 → 순위</small><strong>${i} → ${data.rank[i]}</strong></div><div><small>둘째 시작 위치 → 순위</small><strong>${j} → ${data.rank[j]}</strong></div><div><small>${i===j?'동일 접미사의 길이':'연결 구간 최솟값'}</small><strong>${q.value}</strong></div></div><p>${i===j?`같은 순위이므로 연결할 간선이 없다. ${m(String.raw`N-i`)}를 직접 반환한다.`:`${m(String.raw`\mathrm{LCP}[${q.left}]`)}는 왼쪽 바깥으로 나가는 간선이므로 제외한다. ${q.left+1}부터 ${q.right}까지 연결을 통과하며 유지되는 길이는 최솟값 ${q.value}이다.`}</p><a href="#suffix-theorem-6">RMQ 동치의 증명 →</a>`;
    const ranks=Array.from({length:q.right-q.left+1},(_,k)=>q.left+k);
    get('[data-query-chain]').innerHTML=ranks.map((r:number,index:number)=>`${index?`<li class="${data.lcp[r]===q.value?'bottleneck':''}">${m(String.raw`\mathrm{LCP}[${r}]=${data.lcp[r]}`)} →</li>`:''}<li>${m(String.raw`r=${r}`)} · 시작 ${data.sa[r]}<code>${data.text.slice(data.sa[r])}</code></li>`).join('');
  ...
  function render(step: number) {
    ...
    if (mode === 'sort') {
      ...
      get('[data-reason]').innerHTML=`<h4>${k.r?'재사용 구간과 새 비교 구간을 분리':'이전 이웃이 없는 경계 경우'}</h4><div class="t-reason-grid"><div><small>원문 위치 → 저장할 순위</small><strong>${m(String.raw`i=${k.i}\to r=${k.r}`)}</strong></div><div><small>초록 · 비교 생략</small><strong>${k.seed}글자</strong></div><div><small>파랑 · 새로 확인</small><strong>${k.h-k.seed}글자</strong></div></div><p>${k.r?`재사용 길이 ${k.seed}는 하한이다. 실제 상대 ${m(String.raw`j=${k.j}`)}는 SA에서 다시 찾는다. 불일치 또는 한쪽 끝까지 추가로 확인한 뒤 ${m(String.raw`\mathrm{LCP}[${k.r}]=${k.h}`)}를 기록한다.`:`${m(String.raw`\operatorname{rank}[i]=0`)}에는 이전 이웃이 없다. ${m(String.raw`\mathrm{LCP}[0]=0,\ h=0`)}으로 초기화한다.`}</p><p>다음은 정렬상 다음 행이 아니라 원문 위치 ${m(String.raw`i+1`)}이다.</p><a href="#suffix-theorem-4">비교 상대가 바뀌어도 ${m(String.raw`h-1`)}이 유지되는 증명 →</a>`;
      get('[data-status]').innerHTML = k.r === 0 ? `${m(String.raw`i=${k.i},\ \operatorname{rank}[i]=0`)}. 사전순 이전 접미사가 없으므로 ${m(String.raw`\mathrm{LCP}[0]=0,\ h=0`)}으로 초기화한다.` : `${m(String.raw`i=${k.i}`)}: 바로 이전 접미사는 ${m(String.raw`j=\mathrm{SA}[${k.r-1}]=${k.j}`)}. ${k.seed}글자는 재사용하고 ${k.h - k.seed}글자를 더 일치시킨다. ${m(String.raw`\mathrm{LCP}[${k.r}]=${k.h}`)}. 다음 i에는 ${m(String.raw`h=${k.next}`)}부터 시작한다.`;
      get('[data-compare]').innerHTML = `<div>${m(String.raw`S_{${k.i}}=`)}${chars(k.i, k.h, k.seed)}</div>` + (k.j >= 0 ? `<div>${m(String.raw`S_{${k.j}}=`)}${chars(k.j, k.h, k.seed)}</div>` : '<div>이전 접미사 없음</div>');
      get('[data-kasai-detail]').innerHTML = `<div><dt>재사용한 h</dt><dd>${k.seed}</dd></div><div><dt>이번 ${m(String.raw`\mathrm{LCP}[${k.r}]`)}</dt><dd>${k.h}</dd></div><div><dt>다음 시작 길이</dt><dd>${k.next}</dd></div>`;
